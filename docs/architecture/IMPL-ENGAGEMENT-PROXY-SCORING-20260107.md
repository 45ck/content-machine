...
**Owner:** content-machine core (see `docs/architecture/SYSTEM-DESIGN-20260104.md`)
...
We keep the existing four-stage pipeline intact (`cm script` → `cm audio` → `cm visuals` → `cm render`) while adding a composable validation stage that consumes artifacts (see `docs/architecture/SYSTEM-DESIGN-20260104.md`).  
We align scoring checks with the V&V framework (schema → deterministic checks → LLM rubric → human review) to keep results testable and explainable (see `docs/guides/VV-FRAMEWORK-20260105.md`).  
...
We implement deterministic checks (cadence, hook constraints, caption density proxies) as V&V Layer 2 gates (see `docs/guides/VV-FRAMEWORK-20260105.md`).  
...
We implement `--gate` mode that exits non-zero on failure to support CI and batch pipelines (see `docs/architecture/IMPL-CODE-QUALITY-GATES-20260105.md`).
...
`cm score` reads `script.json` for hook/structure/beat plan signals (see `docs/architecture/IMPL-PHASE-1-SCRIPT-20260105.md`).  
`cm score` reads `timestamps.json` for timing, WPM proxies, and caption alignment coverage (see `docs/architecture/IMPL-PHASE-2-AUDIO-20260105.md`).  
`cm score` reads `visuals.json` for scene durations and “pattern interrupt” cadence proxies (see `docs/architecture/IMPL-PHASE-3-VISUALS-20260105.md`).  
...
Caption-fit scoring evaluates words-per-second and coverage proxies using `timestamps.json` and caption plan fields from `script.json` (see `docs/architecture/IMPL-PHASE-2-AUDIO-20260105.md`).  
...
Add a rubric definition doc for scoring dimensions and thresholds to align with the quality gate framework (see `docs/architecture/IMPL-CODE-QUALITY-GATES-20260105.md`).
...
Implement caption density and readability checks as fast, local computations to preserve the “under 5 minutes” user experience (see `docs/architecture/SYSTEM-DESIGN-20260104.md`).
...
Add `cm score` as a composable command that reads artifacts and writes `score.json` to an output path (see `docs/architecture/IMPL-PHASE-5-INTEGRATION-20260105.md`).  
Add `--gate` and `--json` output modes consistent with other CLI conventions (see `docs/architecture/SYSTEM-DESIGN-20260104.md`).

---

## 6) Testing Strategy

Unit tests cover schema validation and deterministic checks with small fixture JSON files (see `docs/architecture/IMPL-PHASE-0-FOUNDATION-20260105.md`).  
...
The quality gate primitives referenced here are summarized in `docs/research/quality-gates/00-QUALITY-GATES-SUMMARY-20260106.md` and implemented later via `docs/architecture/IMPL-CODE-QUALITY-GATES-20260105.md`.  
