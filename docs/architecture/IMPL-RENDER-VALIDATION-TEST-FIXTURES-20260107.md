...
Committing binary video fixtures is risky for repo size and contributor friction, so we prefer deterministic generation at test time (per `docs/architecture/IMPL-CODE-QUALITY-GATES-20260105.md`).
...
We extract helper functions (`hasCommand`, `makeColorVideo`) to keep test intent clear and reduce repetition (per `docs/architecture/IMPL-CODE-QUALITY-GATES-20260105.md`).
...
We do not use LLMs for validation fixture testing because the properties are deterministic (per `docs/guides/VV-FRAMEWORK-20260105.md`).
