...
We add a probe-engine switch to `validateVideoPath` in `src/validate/validate.ts` (per `docs/architecture/IMPL-RENDER-VALIDATION-PIPELINE-20260107.md`).  
We surface probe-engine selection via CLI flags in `src/cli/commands/validate.ts` (per `docs/architecture/SYSTEM-DESIGN-20260104.md`).
...
We do not use an LLM for probing because the metadata is deterministic and machine-extractable (per `docs/guides/VV-FRAMEWORK-20260105.md`).
