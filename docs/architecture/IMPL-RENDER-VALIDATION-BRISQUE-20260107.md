...
We integrate the gate into `validateVideoPath` in `src/validate/validate.ts` behind an explicit `quality.enabled` option (per `docs/architecture/IMPL-RENDER-VALIDATION-PIPELINE-20260107.md`).  
...
We validate the report shape via `ValidateReportSchema` in `src/validate/schema.ts` (per `docs/guides/VV-FRAMEWORK-20260105.md`).
