...
We keep all research execution behind explicit flags and separate outputs so `cm generate` and `cm score` remain lightweight and stable (see `docs/architecture/SYSTEM-DESIGN-20260104.md` and `docs/architecture/IMPL-ENGAGEMENT-PROXY-SCORING-20260107.md`).  
...
We unit test manifest parsing and command construction without executing external repos (see `docs/architecture/IMPL-PHASE-0-FOUNDATION-20260105.md`).  
We add a single “smoke adapter” that runs a no-op or minimal script so CI can validate harness wiring without downloading datasets (see `docs/architecture/IMPL-CODE-QUALITY-GATES-20260105.md`).  
...
We use the research harness to learn which proxy signals correlate with offline model predictions while keeping production scoring lightweight (see `docs/architecture/IMPL-ENGAGEMENT-PROXY-SCORING-20260107.md`).  
