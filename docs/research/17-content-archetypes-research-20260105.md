...
export function calculateVolume(level: MusicVolumeEnum): [number, boolean] {
  switch (level) {
    case 'muted':
      return [0, true];
    case 'low':
      return [0.2, false]; // Educational, story
    case 'medium':
      return [0.45, false]; // Balanced
    case 'high':
      return [0.7, false]; // Brainrot, meme
  }
}
```
...
// vendor/ShortGPT/shortGPT/editing_framework/editing_steps/make_caption.json
{
  "caption": {
    ...
    "actions": [{ "type": "screen_position", "param": { "pos": "center" } }]
  }
}
```
...
| [captacity/**init**.py](../vendor/captacity/captacity/__init__.py)                                      | Caption styling API                     |
