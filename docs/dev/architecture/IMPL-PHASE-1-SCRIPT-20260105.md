...
export const SceneSchema = z.object({
  ...
  narration: z.string().min(10, 'Narration too short').max(500, 'Narration too long'),
  visualDirection: z
    .string()
...
export const ScriptOutputSchema = z.object({
  ...
  scenes: z.array(SceneSchema).min(3, 'At least 3 scenes required').max(8, 'Maximum 8 scenes'),
...
function countWords(script: ScriptOutput): number {
  return script.scenes.reduce((acc, scene) => acc + scene.narration.split(/\s+/).length, 0);
}
```
  ...
  return new Command('script')
    ...
    .action(async (topic, options) => {
      ...
      try {
        ...
        spinner.succeed(
          `Script generated (${result.script.scenes.length} scenes, ${result.script.metadata.wordCount} words)`
        );
