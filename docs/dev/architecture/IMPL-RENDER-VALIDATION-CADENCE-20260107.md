...
We keep this gate severity as `warning` so it can guide iteration without blocking uploads unless explicitly desired (per `docs/dev/architecture/IMPL-ENGAGEMENT-PROXY-SCORING-20260107.md`).
...
We wire cadence as an optional gate in `src/validate/validate.ts` and expose it via `--cadence` flags in `src/cli/commands/validate.ts` (per `docs/dev/architecture/SYSTEM-DESIGN-20260104.md`).
...
We do not use LLM evaluation for cadence because the signal is mechanical and measurable (per `docs/dev/guides/VV-FRAMEWORK-20260105.md`).
