...
vi.mock('../../../../src/quality-score/scorer', () => ({
  scoreQuality: vi.fn(),
}));
...
describe('cli qa command', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.resetModules();
  });

  it('prints evaluation fixes in human mode', async () => {
    ...
    const { qaCommand } = await import('../../../../src/cli/commands/qa');
    await qaCommand.parseAsync(
      ['--input', 'video.mp4', '--skip-score', '--output', 'qa.evaluate.json'],
      { from: 'user' }
    );
    ...
    expect(capture.stderr.join('')).toContain('Fix (rate): Run: cm setup whisper --model base');
    expect(capture.stdout.join('')).toContain('qa.evaluate.json');
  });
});
