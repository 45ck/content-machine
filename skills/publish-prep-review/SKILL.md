...
description: Score the script, validate the rendered video, and produce publish metadata so the harness can decide whether a short is ready to upload.
allowedTools:
  - shell
  - read
  - write
...
inputs:
  - name: videoPath
    ...
    required: true
  - name: scriptPath
    description: ScriptOutput artifact that produced the render.
    required: true
  - name: platform
...
- A harness needs deterministic pass/fail artifacts before human review.
