...
export interface MatchVisualsOptions {
  ...
  mock?: boolean;
  /**
   * Optional progress callback for CLI UX (stage percent + sub-phase).
   * Progress is reported as 0..1.
   */
...
export async function matchVisuals(options: MatchVisualsOptions): Promise<VisualsOutput> {
  ...
  const safeProgress = (event: VisualsProgressEvent): void => {
    try {
      options.onProgress?.({
        ...event,
  ...
  if (options.mock) {
    safeProgress({
      phase: 'mock',
      progress: 1,
      message: 'Mock visuals generated',
    });
  ...
  safeProgress({ phase: 'complete', progress: 1, message: 'Visual matching complete' });
  return validated;
}
