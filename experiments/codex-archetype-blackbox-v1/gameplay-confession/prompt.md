...
3. write `outputs/work/script.json` in the same shape as
   `reference/manual-script.example.json`
4. run `script-to-audio`
5. write `outputs/work/visuals.json` to cover the full audio duration
   using only `assets/top/*.mp4` and the gameplay clip
6. run `video-render` using
   `reference/video-render.request.example.json` exactly as written
   unless you have a better local cached browser path
7. run `publish-prep` using
   `reference/publish-prep.request.example.json`
8. if review fails, fix the real problem once and rerun
9. write `outputs/final/report.md`
